...
# Plot Histogram
def histogram_plot(df: pd.DataFrame) ->  str:
...
# Plot Scatterplots
def scatter_plot(df: pd.DataFrame) ->  str:
    max_rows = 1500
    feature_columns = [col for col in df.columns if col != "label"]
    df_ko = df[df["label"] == "KO"]
    df_ok = df[df["label"] == "OK"]
    ...
    for idx, col in enumerate(feature_columns):
        for in_idx, in_col in enumerate(feature_columns):   
            if idx > in_idx:
                ...
                plt.scatter(df_ko_scatter[col], df_ko_scatter[in_col], label='KO', alpha=0.3, color=color_ko, s=5)
                plt.scatter(df_ok_scatter[col], df_ok_scatter[in_col], label='OK', alpha=0.3, color=color_ok, s=5)
                ...
                x_vals_ko = np.linspace(df_ko[col].min(), df_ko[col].max(), 100).reshape(-1, 1)
                ...
                x_vals_ok = np.linspace(df_ok[col].min(), df_ok[col].max(), 100).reshape(-1, 1)
